...
static audio_signal* parse_params(char** argv) {
    ...
    aud_signal->modulating_params = malloc(sizeof(wave_params));

    aud_signal->out_file = argv[index++];
    aud_signal->carrier_fn = parse_wave_fn(argv[index++]);
    aud_signal->modulating_fn = parse_wave_fn(argv[index++]);
    aud_signal->mod_type = parse_mod_type(argv[index++]);
    ...
    aud_signal->modulating_params->amplitude = strtod(argv[index++], NULL);
    aud_signal->modulating_params->frequency = strtod(argv[index++], NULL);
    aud_signal->modulating_params->dutyCycle = strtod(argv[index++], NULL);

    aud_signal->carrier_params->sample_rate = aud_signal->modulating_params->sample_rate = aud_signal->sample_rate = atoi(argv[index++]);
    aud_signal->carrier_params->initial_phase = aud_signal->modulating_params->initial_phase = strtod(argv[index++], NULL);
    aud_signal->signal_duration = strtod(argv[index++], NULL);

    aud_signal->num_samples = aud_signal->sample_rate * aud_signal->signal_duration;
